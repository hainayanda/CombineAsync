...
extension Sequence where Element: Publisher {
    ...
    typealias Merged = Publishers.MergeMany<Element>
    typealias MergedFirsts = Publishers.MergeMany<Publishers.First<Element>>
    typealias Collected = Publishers.Collect<Merged>
    typealias CollectedFirsts = Publishers.Collect<MergedFirsts>
    ...
    public func convertToAsync() async throws -> [Element.Output] {
        try await mergedFirsts().asynchronously()
    }
    
    // MARK: Internal methods
    
    func merged() -> Collected {
        Publishers.MergeMany(self)
            .collect()
    }
    
    func mergedFirsts() -> CollectedFirsts {
...
extension Sequence {
    ...
    public func asyncMap<Mapped>(_ mapper: @escaping (Element) async throws -> Mapped) async throws -> [Mapped] {
    ...
    /// - Returns: An array containing the transformed elements of this sequence
    public func asyncCompactMap<Mapped>(_ mapper: @escaping (Element) async throws -> Mapped?) async throws -> [Mapped] {
        try await convertToIndexedFutures(mapper)
            .convertToAsync()
            .sorted { $0.index < $1.index }
